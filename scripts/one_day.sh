...
# DOESNT WORK, here is my first attempt but I realise that this only adds each number to the next and not the other combinations
...
# WORKS, here is the second attempt, messy but it works
for i in "${!my_array[@]}"; do
    a="${my_array[i]}"
...
# WORKS, This is a horrible way to do it and made my laptop run hard but yay it works first try!
for i in "${!my_array[@]}"; do
    a="${my_array[i]}"
    for j in "${!my_array[@]}"; do
        b="${my_array[j]}"
